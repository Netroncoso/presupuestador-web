...
export default function GestionEquipamientosBase() {
  ...
  const [formData, setFormData] = useState({ 
    nombre: '', 
    tipo_equipamiento_id: null as number | null,
    precio_referencia: 0,
    activo: 1
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setModalOpen(false);
      setEditando(null);
      setFormData({ nombre: '', tipo_equipamiento_id: null, precio_referencia: 0, activo: 1 });
      cargarEquipamientos();
    } catch (error: any) {
      notifications.show({
        title: 'Error',
  ...
  const handleEdit = (equipo: Equipamiento) => {
    ...
    setFormData({ 
      ...
      tipo_equipamiento_id: tipos.find(t => t.nombre === equipo.tipo)?.id || null,
    ...
    <Stack gap="md">
      ...
      >
        <Stack gap="md">
          ...
          <Select
            label="Tipo"
            value={formData.tipo_equipamiento_id?.toString() || null}
            onChange={(value) => setFormData({ ...formData, tipo_equipamiento_id: value ? parseInt(value) : null })}
