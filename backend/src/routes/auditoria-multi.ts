...
import { authenticateToken, requireGerenciaPrestacional, requireGerenciaComercial, requireGerenciaGeneral, requireAnyGerencia } from '../middleware/auth';
 ...
 *   put:
 *     summary: Aprobar presupuesto (G. Comercial)
 *     tags: [Auditoría Multi-Gerencial]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Derivado a G. Comercial
 * /api/auditoria-multi/administrativa/rechazar/{id}:
 *   put:
 *     summary: Rechazar presupuesto (G. Comercial)
 *     tags: [Auditoría Multi-Gerencial]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
router.put('/prestacional/derivar/:id', requireGerenciaPrestacional, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { comentario } = req.body;
...
router.put('/comercial/aprobar/:id', requireGerenciaComercial, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const id = parseInt(req.params.id);
  const { comentario } = req.body;
...
router.put('/comercial/aprobar-condicional/:id', requireGerenciaComercial, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { motivo } = req.body;
...
router.put('/comercial/rechazar/:id', requireGerenciaComercial, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { comentario } = req.body;
...
router.put('/comercial/observar/:id', requireGerenciaComercial, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { comentario } = req.body;
...
router.put('/comercial/escalar/:id', requireGerenciaComercial, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { motivo } = req.body;
...
router.put('/general/devolver/:id', requireGerenciaGeneral, validatePresupuestoId, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!gerenciaDestino || !['prestacional', 'comercial'].includes(gerenciaDestino)) {
